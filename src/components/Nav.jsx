    ...
    return (
        <div className={`${styles.container}`}>
            <div className={`${styles.logo}`}>
                ...
                <h3 style={{ flexShrink: 0, fontSize: '24px', color: '#2B2579', fontWeight: 700 }}>Matrix AI Dashboard</h3>
            </div>
            ...
            <div className={`${styles.switch}`}>
                {btnArray.map((item, index) => (
                    <button
                        key={index}
                        className={`${selected === index ? styles.selected : styles.unselected}`}
                        onClick={() => {
                            setSelect(index);
                            item.click();
                        }}
            ...
            <div className={`${styles.menu}`}>
                {menuIconArray.map((item, index) => (
                    <Image key={index} src={item.icon} alt={item.alt} style={{cursor:'pointer'}}/>
                ))}
                <span style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                    <Image src={Avtar} alt="user" style={{ height: '45px', objectFit: 'cover' }} />
